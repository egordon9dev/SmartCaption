...
def renderCaption(frame, box, color, message):
    ...
    cv2.putText(frame, text=message, org=(textOriginX, textOriginY + textHeight - height//2), fontFace=FONT,
            fontScale=FONT_SCALE, color=color, thickness=FONT_THICKNESS)
    if debug:
