...
outputDirectory = "data/rock_and_kheart/"
videoURL = "https://www.youtube.com/watch?v=E6LpBIwGyA4"

debugOut = subprocess.DEVNULL
debugErr = subprocess.DEVNULL

downloadCommand = ["youtube-dl", "-f", "18", "-o", outputDirectory + "%(title)s.%(ext)s", "--write-sub",
...
times = [[parseTime(s) for s in lines[1].split("-->")] for lines in split[2:]]
# convert to frame numbers
times = [(math.floor(startTime * fps), math.ceil(endTime * fps)) for startTime, endTime in times]
# parse into caption tuples
captions = [Caption(None, ' '.join(lines[2:]), startTime, endTime, lines[0].strip()) for (startTime, endTime), lines in zip(times, split[2:])]
